...
* 23% of the people leave the company.'
...
# Changing the order of the columns, putting target column in first
df <- df[,c(7,1,2,3,4,5,6,8,9,10)]
...
# Selecting only the columns from Feature Engineering section
df2 <- df %>% select(7, 1:5)
df2$left_company <- as.factor(df2$left_company)

# New Train and Test Split
df_split <- initial_split(df2, strata = left_company)
'<Analysis/Assess/Total>
<11250/3749/14999>'
# Transforming the split in two new dataframes for Train and Test
df_train <- training(df_split)
df_test <- testing(df_split)

# SMOTE - balancing the training dataset
df_train <- SMOTE(left_company ~., data= df_train, perc.over = 190, k=5)
table(df_train$left_company)
...
# Creating a Bootstrap for training with many samples
df_boot <- bootstraps(df_train)


#### Random Forest

# Creating instance of the RF Model
rf_spec <- rand_forest() %>%
...
'This model did very well, with 98% of accuracy
...
xgb_spec <- boost_tree(trees = 250, mtry = 10,
                       min_n = 2, tree_depth = 6) %>%
  set_engine('xgboost') %>%
  set_mode('classification')
...
# Add a model
xgb_rs <- xgb_spec %>%
  fit(left_company~., data = df_train) %>%
  predict(df_train[,-1])

# Confusion Matrix
confusionMatrix(df_train$left_company, xgb_rs$.pred_class)
roc(factor(df_train$left_company, ordered=T),
    factor(xgb_rs$.pred_class, ordered=TRUE),
    main = 'AUC for XGB model', plot=T)
'Accuracy of 99%
ROC = 99%'

# Add Formula to Workflow for Last Fit 
xgb_wf <- workflow() %>%
  add_formula(left_company ~ .)
...
xgb_final %>%
  collect_metrics()
'This model did very well, with 99% of accuracy'

# Confusion Matrix 
xgb_final %>% collect_predictions() %>% conf_mat(left_company, .pred_class)
'         Truth
Prediction    0    1
         0 2844   27
         1   13  865'
...
###########################
# Saving the Performance to CSV file
#-----------------------------------------------------------

result <- data.frame(left_company = df_test$left_company, prediction = xgb_final$.predictions)
result$assessment <- ifelse(result$left_company == result$prediction..pred_class, 'Correct', 'Wrong')

write_csv(result, 'Predictions.csv')
